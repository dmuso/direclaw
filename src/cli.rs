...
fn cmd_update(args: &[String]) -> Result<String, String> {
    if args.is_empty() || args[0] == "check" || args[0] == "apply" {
        return Err(
            "update is not implemented in this build; use GitHub Releases for upgrade. remediation: visit release notes and install the target binary manually".to_string(),
        );
    }
