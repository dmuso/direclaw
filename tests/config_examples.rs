...
fn settings_examples_parse_and_validate_cross_references() {
    let minimal = Settings::from_path(Path::new(
    ...
    let full = Settings::from_path(Path::new("docs/spec/examples/settings/full.settings.yaml"))
...
fn orchestrator_examples_parse_and_validate_required_fields() {
    let settings = Settings::from_path(Path::new("docs/spec/examples/settings/full.settings.yaml"))
        .expect("load full settings");

    let engineering = OrchestratorConfig::from_path(Path::new(
    ...
    let product = OrchestratorConfig::from_path(Path::new(
    ...
    let minimal_settings = Settings::from_path(Path::new(
        "docs/spec/examples/settings/minimal.settings.yaml",
    ))
    .expect("load minimal settings");
    let minimal = OrchestratorConfig::from_path(Path::new(
