...
#[test]
fn scheduled_trigger_surfaces_completion_failures_as_routing_errors() {
    let temp = tempdir().expect("tempdir");
    let workspaces_root = temp.path().join("workspaces");
    let private_workspace = workspaces_root.join("main");
    write_orchestrator(&private_workspace);
    ...
    std::fs::create_dir_all(runtime_root.join("automation/scheduler_state.json"))
        .expect("poison scheduler state path");

    let envelope = ScheduledTriggerEnvelope {
        job_id: "job-complete".to_string(),
    ...
    let inbound = IncomingMessage {
        ...
        channel_profile_id: None,
        sender: "scheduler:main".to_string(),
        sender_id: "job-complete".to_string(),
        message: serde_json::to_string(&envelope).expect("serialize envelope"),
        timestamp: 1_700_000_000,
        message_id: "exec-complete".to_string(),
        conversation_id: Some("scheduler:job-complete".to_string()),
        is_direct: false,
        is_thread_reply: false,
        is_mentioned: false,
        files: vec![],
    ...
    .expect_err("completion failure should be surfaced");
    assert!(
        err.to_string()
            .contains("scheduled execution completion failed"),
        "unexpected error: {err}"
    );
}
