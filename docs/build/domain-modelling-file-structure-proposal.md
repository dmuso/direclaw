...
- 2026-02-15 14:07 - Extracted channel-profile command handling from `src/commands.rs` into `src/app/command_handlers/channel_profiles.rs` (`cmd_channel_profile` and option parsing), exported via `app::command_handlers`, delegated CLI `channel-profile` through the new handler, and added integration coverage for `direclaw::app::command_handlers::channel_profiles::cmd_channel_profile`.
- 2026-02-15 14:34 - Extracted daemon/supervisor command handling from `src/commands.rs` into `src/app/command_handlers/daemon.rs` (`cmd_start`, `cmd_stop`, `cmd_restart`, `cmd_status`, `cmd_logs`, and `cmd_supervisor`, including slack-profile health/status projection helpers), exported via `app::command_handlers`, delegated CLI daemon verbs through the new handler, and added integration coverage for `direclaw::app::command_handlers::daemon::*` access path.
- 2026-02-15 14:15 - Extracted orchestrator command handling from `src/commands.rs` into `src/app/command_handlers/orchestrators.rs` (`cmd_orchestrator` and all orchestrator subcommands), exported via `app::command_handlers`, delegated CLI `orchestrator` through the new handler, promoted shared config save/load helpers in `commands` to `pub(crate)` for reuse, and added integration coverage for `direclaw::app::command_handlers::orchestrators::cmd_orchestrator`.
- 2026-02-15 14:52 - Extracted workflow command handling from `src/commands.rs` into `src/app/command_handlers/workflows.rs` (`cmd_workflow` plus `--input key=value` parsing), exported via `app::command_handlers`, delegated CLI `workflow` through the new handler, promoted shared timestamp helpers in `commands` to `pub(crate)` for handler reuse, and added integration coverage via `tests/app_command_handlers_workflows.rs` for `direclaw::app::command_handlers::workflows::cmd_workflow`.
- 2026-02-15 14:22 - Extracted orchestrator-agent command handling from `src/commands.rs` into `src/app/command_handlers/agents.rs` (`cmd_orchestrator_agent` with `list|add|show|remove|reset` subcommands), exported via `app::command_handlers`, delegated CLI `agent`/`orchestrator-agent` through the new handler import, and added integration coverage via `tests/app_command_handlers_agents.rs` plus `tests/config_typing_foundations.rs` for `direclaw::app::command_handlers::agents::cmd_orchestrator_agent`.
