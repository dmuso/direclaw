...
- 2026-02-15 18:45 - Moved supervisor worker-event state application concerns (`apply_worker_event`, `WorkerHealth` state transitions, and worker-event log shaping) from `src/runtime/supervisor.rs` to `src/runtime/worker_registry.rs`; kept compatibility export of `WorkerHealth` via `runtime::supervisor`, updated supervisor to delegate to `worker_registry::apply_worker_event`, and added integration coverage for the new `direclaw::runtime::worker_registry::apply_worker_event` module API.
- 2026-02-16 10:40 - Removed the transitional `src/function_bridge.rs` compatibility module by lifting function-dispatch re-exports (`V1_FUNCTIONS`, `FunctionExecutionContext`, `execute_function_invocation`) to `src/lib.rs`, updated `src/orchestration/function_registry.rs` to consume crate-root exports, and added orchestration boundary coverage to block reintroduction of `crate::function_bridge` imports.
- 2026-02-15 18:44 - Moved runtime worker event typing (`WorkerEvent`) from `src/runtime/mod.rs` to `src/runtime/worker_registry.rs`, re-exported via `runtime` for compatibility, and added integration coverage for the new `direclaw::runtime::worker_registry::WorkerEvent` module path.
- 2026-02-15 18:52 - Expanded `src/runtime/heartbeat_worker.rs` to own heartbeat scheduling resolution (`configured_heartbeat_interval`), updated `src/runtime/channel_worker.rs` to consume the heartbeat module for interval selection, and added integration coverage for the new `direclaw::runtime::heartbeat_worker::configured_heartbeat_interval` module surface.
- 2026-02-16 10:16 - Removed the remaining legacy `tui` compatibility Rust module (`src/tui/mod.rs`, `src/tui/setup.rs`) and `lib.rs` export (`pub mod tui;`), keeping setup entry points under `src/setup/*`; added module-layout coverage to prevent reintroducing the legacy path.
...
- 2026-02-16 10:27 - Moved workflow-step detail row projection (`workflow_step_menu_rows`) out of `src/setup/actions.rs` into `src/setup/screens.rs` so setup TUI row/view composition stays grouped with other screen helpers, updated setup actions to consume the extracted API, and added integration coverage for the new `direclaw::setup::screens::workflow_step_menu_rows` module surface.
- 2026-02-16 10:33 - Removed worker-loop sleep orchestration from `src/runtime/mod.rs` by localizing `sleep_with_stop` to `src/runtime/channel_worker.rs` and `src/runtime/queue_worker.rs` (the only call sites), keeping runtime root focused on module boundaries/types; added a runtime boundary guard test in `tests/runtime_boundary_module.rs` to prevent reintroducing loop-helper logic into `runtime/mod.rs`.
