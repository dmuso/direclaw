...
2026-02-20 12:57 - Updated selector-driven workflow-start run IDs to `run-<base36_ts>-<rand36_4>` using CSPRNG-backed suffix generation in `route_selector_action`, added TDD coverage for the compact ID format, and updated queue concurrency e2e assertions/scripts to resolve run IDs from persisted run metadata rather than legacy `run-sel-*` patterns.
2026-02-20 13:03 - Added reserved attempt-root output target enforcement in `resolve_step_output_paths` to block `prompt.md`, `context.md`, and `provider_invocation.json` collisions, introduced TDD coverage for all reserved names, and revalidated with full `fmt`, `clippy -D warnings`, and `cargo test --all` in `nix-shell`.
2026-02-20 13:08 - Reduced step-attempt provider instruction path token overhead by deduplicating shared attempt-root prefixes in `provider_instruction_message` (now referencing `prompt.md`/`context.md` from a single directory path), added TDD unit coverage for both shared-root and fallback path rendering, and revalidated with full `fmt`, `clippy -D warnings`, and `cargo test --all` in `nix-shell`.
...
2026-02-20 13:27 - TODO [status: todo] Align CLI `workflow run` run-id generation with the compact `run-<base36_ts>-<rand36_4>` format (or explicitly narrow scope in this plan if intentional). Instruction: once complete, update this item to `status: done` and record the chosen scope decision and validation.
